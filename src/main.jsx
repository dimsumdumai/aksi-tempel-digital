...
const USERS=['Dimas','Siti','Imelda','Lina','Luisi'];
...
function App(){
 ...
 useEffect(()=>{fetch('/database-gaspoll.xlsx').then(r=>r.arrayBuffer()).then(b=>{const wb=XLSX.read(b,{type:'array',cellDates:true});const rows=XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]],{defval:''});setDb(rows);}).catch(console.error).finally(()=>setLoadingDb(false));},[]);
 ...
 function markPrinted(id,status='Dicetak'){setEntries(v=>v.map(e=>e.id===id?{...e,print_status:status}:e));setSelectedNotice(v=>v?.id===id?{...v,print_status:status}:v);}
